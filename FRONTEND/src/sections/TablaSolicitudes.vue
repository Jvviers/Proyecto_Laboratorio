...
<template>
  <header
    v-if="!validSession"
    class="flex flex-col justify-center items-center gap-2 py-12"
  >
  ...
  <section
    v-if="validSession"
    class="flex flex-col justify-center items-center w-full px-8 md:px-32 gap-6 py-12"
  >
      ...
      <table class="table-auto bg-white">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ...
          <tr v-for="request in paginatedRequests" :key="request.id">
            ...
            <td>
              <select
                v-if="request.tipo_form != 'asesoria'"
                v-model="request.ref_enc"
                @change="
                  updateEncargado(
                    request.id,
                    request.ref_enc,
            ...
            <td>
              ...
              <select
                v-if="request.tipo_form == 'impresion'"
                v-model="request.estado"
                @change="
                  updateState(
                    request.id,
              ...
              <select
                v-if="request.tipo_form == 'laboratorio'"
                v-model="request.estado"
                @change="
                  updateState(
                    request.id,
            ...
            <td class="flex justify-center items-center py-4 gap-4">
              <button
                @click="eliminarSolicitud(request.id)"
                type="button"
                class="delete"
              >
                <svg
              ...
              <a
                v-if="request.tipo_form === 'impresion'"
                :href="`${BACKEND_URL}/download/${request.id}`"
                target="_blank"
