...
const handleSubmit = () => {
  client.uploadFile(fileInput.value).then((file) => {
    fileData.value = file;
...
<template>
  <div
    v-if="accessMessage"
    class="fixed top-32 right-5 bg-green-500 text-white py-2 px-4 rounded shadow-lg z-50"
  >
    {{ accessMessage }}
  </div>
  ...
  >
    ...
    <div
      class="flex flex-col justify-center items-center gap-4 mx-auto px-10 py-6"
    >
    ...
    <div class="flex flex-wrap gap-8 px-4 justify-evenly w-ful">
      <div class="flex flex-wrap gap-8 px-4 justify-evenly w-full">
        ...
        <div class="flex flex-col gap-1 justify-start items-center">
          <div
            class="flex flex-col justify-start items-center gap-3 md:gap-8 p-2"
          >
            <h2 class="font-bold text-center text-2xl">Archivo</h2>
            <button type="button" class="button relative overflow-hidden">
              Seleccionar Archivo
              <input
                type="file"
                title=""
