<script setup>
import { ref } from 'vue';
import AgregarNoticia from './AgregarNoticia.vue';
import EliminarNoticia from './EliminarNoticia.vue';
...
<template>
    <div class="flex flex-col justify-center items-center gap-2 px-8 py-4 w-[200px] border border-gray-300 shadow-md rounded">
        <h2 class="text-lg font-bold text-utal">NOTICIAS</h2>
	...
	<div v-if="modalAgregar" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-10">
		<div class="bg-white p-4 rounded shadow-lg relative w-3/4 md:w-1/2">
			<AgregarNoticia :isModalOpen="modalAgregar" @close-modal="toggleModalAgregar" />
		</div>
	</div>
    ...
    <!-- <div v-if="modalEditar" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-10">
		<div class="bg-white p-4 rounded shadow-lg relative">
			<EditarNoticia :isModalOpen="modalEditar" @close-modal="toggleModalEditar" />
		</div>
	</div> -->
</template>