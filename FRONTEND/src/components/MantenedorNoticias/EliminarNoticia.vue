...
<template>
    <section class="flex flex-col justify-center items-center gap-6 py-4 px-12 w-full max-h-[80vh]">
        ...
        <div class="flex flex-col justify-start items-stretch w-full gap-1 overflow-y-scroll">
            <li v-for="noticia in noticias" :key="noticia.id" class="flex justify-between items-center px-2 w-full">
                <span class="bg-gray-50 border border-gray-200 px-2 py-1 rounded w-full max-w-[200px] sm:max-w-[350px] md:max-w-[480px]">{{ noticia.titulo }}</span>
                <button class="ml-6" type="button" @click="eliminarNoticia(noticia.id)">
