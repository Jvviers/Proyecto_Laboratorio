...
<template>
    <section class="flex flex-col justify-center items-center gap-6 py-4 px-12 w-full max-h-[80vh]">
        <h2 class="text-2xl font-bold text-center text-utal">ELIMINAR LOGO DE LABORATORIO</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 overflow-y-scroll">
            <li v-for="logo in logos" :key="logo.id"
                class="flex justify-between items-center p-2 w-full border border-gray-300 rounded shadow-md relative bg-neutral-200">
                <img :src="logo.url" :alt="'logo ' + logo.id" class="aspect-square w-full" />
                <button class="absolute top-0 right-0 p-[5px] m-3 rounded-full bg-black/15 opacity-0" type="button" @click="deleteLogo(logo.id)">
